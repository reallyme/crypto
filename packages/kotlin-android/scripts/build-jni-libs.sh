...
if [[ -z "${ANDROID_NDK_HOME:-}" ]]; then
  ANDROID_NDK_HOME="$(find "${ANDROID_SDK_ROOT_VALUE}/ndk" -maxdepth 1 -mindepth 1 -type d | sort -V | tail -n 1)"
  export ANDROID_NDK_HOME
fi
if [[ -z "${ANDROID_NDK_HOME}" || ! -d "${ANDROID_NDK_HOME}" ]]; then
  echo "Android NDK not found under ${ANDROID_SDK_ROOT_VALUE}/ndk" >&2
  exit 1
fi
