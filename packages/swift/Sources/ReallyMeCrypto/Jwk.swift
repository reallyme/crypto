...
/// Native JWK conversion for package consumers.
///
/// JWK is JSON and base64url over public key bytes. Keeping it in the Swift
/// facade avoids serializing JSON through the C ABI and makes the package lane
/// independently checkable against `vectors/jwk.json`.
public enum ReallyMeJwk {
    public static func toJwk(
        ...
        if spec.kty == "EC" {
            ...
            return ReallyMeJwkDocument(
                ...
                keyUse: spec.keyUse,
                crv: spec.crv,
                x: Array(uncompressed[1..<33]).base64UrlEncodedString(),
                y: Array(uncompressed[33..<65]).base64UrlEncodedString(),
                publicKey: nil
            )
        }
    ...
    private static func fromJwkObject(_ object: [String: Any]) throws -> ReallyMeJwkKey {
        ...
        if spec.kty == "EC" {
            ...
            publicKey = try compressEcPublicKey(
                algorithm: algorithm,
                x: Array(base64UrlDecode(encodedX)),
                y: Array(base64UrlDecode(encodedY))
            )
        } else if spec.kty == "AKP" {
            guard let encodedPublicKey = object["pub"] as? String else {
                throw ReallyMeCryptoError.invalidInput
            }
            publicKey = Array(try base64UrlDecode(encodedPublicKey))
        } else {
    ...
    private static func base64UrlDecode(_ encoded: String) throws -> [UInt8] {
