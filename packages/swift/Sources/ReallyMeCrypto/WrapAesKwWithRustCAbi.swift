...
private typealias AesKwWrapKeyFunction =
  @convention(c) (
    UnsafePointer<UInt8>?,
    Int,
    UnsafePointer<UInt8>?,
...
private typealias AesKwUnwrapKeyFunction =
  @convention(c) (
    UnsafePointer<UInt8>?,
    Int,
    UnsafePointer<UInt8>?,
...
public struct ReallyMeRustCAbiAesKw: Sendable {
  ...
  public func wrapKey(
    ...
    guard outputLength.overflow == false else {
  ...
  private func validateWrappingKey(_ key: [UInt8], algorithm: ReallyMeKeyWrapAlgorithm)
  ...
  private func validateKeyData(_ keyData: [UInt8]) throws(ReallyMeCryptoError) {
  ...
  private func validateWrappedKey(_ wrappedKey: [UInt8]) throws(ReallyMeCryptoError) {
