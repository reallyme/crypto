...
typealias ReallyMeCryptoOperationResponseFunction =
  @convention(c) (
    UnsafePointer<UInt8>?,
    Int,
...
struct ReallyMeRustCAbiOperationResponseProcessor {
  ...
  init(library: ReallyMeRustCAbiLibrary) throws(ReallyMeCryptoError) {
  ...
  func processOperationResponse(_ request: [UInt8]) throws(ReallyMeCryptoError) -> [UInt8] {
    try process(request, with: processOperationResponseFunction)
  }

  func processOperationResponseJson(_ requestJson: [UInt8]) throws(ReallyMeCryptoError) -> [UInt8] {
...
extension ReallyMeCrypto {
  ...
  public func processOperationResponse(_ request: [UInt8]) throws(ReallyMeCryptoError) -> [UInt8] {
    guard let library = providers.rustCAbiLibrary else {
      throw ReallyMeCryptoError.providerFailure
    }
    return try ReallyMeRustCAbiOperationResponseProcessor(library: library)
  ...
  public func processOperationResponseJson(_ requestJson: [UInt8]) throws(ReallyMeCryptoError)
    -> [UInt8]
  {
    guard let library = providers.rustCAbiLibrary else {
      throw ReallyMeCryptoError.providerFailure
    ...
    return try ReallyMeRustCAbiOperationResponseProcessor(library: library)
      .processOperationResponseJson(requestJson)
  }
}
