...
struct ConcatKdfVector: Decodable {
...
extension ReallyMeCryptoTests {
  ...
  func testConfiguredProviderContextRoutesRustBackedAlgorithmsWhenConfigured() throws {
    ...
    let plaintext = try Self.base64UrlBytes(
      "UmVhbGx5TWUgQUVTLTI1Ni1HQ00tU0lWIGNvbmZvcm1hbmNlIHZlY3Rvcg")
    ...
    XCTAssertEqual(
      try crypto.open(
        .aes256GcmSiv, key: key, nonce: nonce, aad: aad, ciphertextWithTag: ciphertext),
      plaintext
    )
    XCTAssertThrowsError(
      try crypto.seal(
        .aes256GcmSiv, key: Array(key.dropLast()), nonce: nonce, aad: aad, plaintext: plaintext)
    ) { error in
      XCTAssertEqual(error as? ReallyMeCryptoError, .invalidInput)
    }
    var tampered = ciphertext
    ...
    let rsaPublicKeyDer = try Self.base64UrlBytes(
      ReallyMeCryptoRustCAbiTests.rsaPublicKeyDerBase64Url)
    let rsaMessage = try Self.base64UrlBytes("UmVhbGx5TWUgc2lnbmF0dXJlIGNvbmZvcm1hbmNlIHZlY3Rvcg")
    let rsaSignature = try Self.base64UrlBytes(
      ReallyMeCryptoRustCAbiTests.rsaPkcs1Sha256SignatureBase64Url)
    try crypto.verify(
      .rsaPkcs1v15Sha256,
    ...
    let bip340SecretKey = try Self.base64UrlBytes(
      ReallyMeCryptoRustCAbiTests.bip340SecretKeyBase64Url)
    let bip340PublicKey = try Self.base64UrlBytes(
      ReallyMeCryptoRustCAbiTests.bip340PublicKeyBase64Url)
    ...
    let bip340Signature = try Self.base64UrlBytes(
      ReallyMeCryptoRustCAbiTests.bip340SignatureBase64Url)
    XCTAssertEqual(
      try crypto.deriveBip340SchnorrPublicKey(secretKey: bip340SecretKey), bip340PublicKey)
    XCTAssertEqual(
      try crypto.sign(
        .bip340SchnorrSecp256k1Sha256,
    ...
    for ecdsaCase in ecdsaCases {
      let (
        algorithm, secretKey, publicKey, message, expectedSignature, publicKeyLength,
        secretKeyLength
      ) =
        ecdsaCase
