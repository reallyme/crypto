...
/**
 * JWK conversion for Kotlin/JVM and Android package consumers.
 *
...
public object ReallyMeJwk {
    ...
    public fun toJwk(
        ...
        if (spec.kty == "EC") {
            ...
            return ReallyMeJwkDocument(
                ...
                keyUse = spec.keyUse,
                crv = spec.crv,
                x = codecBase64urlEncode(uncompressed.copyOfRange(1, 33)),
                y = codecBase64urlEncode(uncompressed.copyOfRange(33, 65)),
                publicKey = null,
            )
        }

        val encodedPublicKey = codecBase64urlEncode(publicKey)
        return ReallyMeJwkDocument(
            algorithm = algorithm,
            kty = spec.kty,
            alg = spec.alg,
    ...
    public fun toJcs(jwk: ReallyMeJwkDocument): String = codecOperation {
    ...
    public fun fromJwkJson(json: String): ReallyMeJwkKey {
        ...
        val publicKey = when (spec.kty) {
            "EC" -> compressEcPublicKey(
                algorithm,
                codecBase64urlDecode(objectMap["x"] ?: throw ReallyMeCryptoException.InvalidInput()),
                codecBase64urlDecode(objectMap["y"] ?: throw ReallyMeCryptoException.InvalidInput()),
            )
            "AKP" -> codecBase64urlDecode(objectMap["pub"] ?: throw ReallyMeCryptoException.InvalidInput())
            "OKP" -> codecBase64urlDecode(objectMap["x"] ?: throw ReallyMeCryptoException.InvalidInput())
            else -> throw ReallyMeCryptoException.UnsupportedAlgorithm()
        }
    ...
    private fun codecBase64urlDecode(encoded: String): ByteArray =
        codecOperation { ReallyMeCodec.base64urlDecode(encoded) }
