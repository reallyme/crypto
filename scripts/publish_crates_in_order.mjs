...
const allowAlreadyPublished = process.env.REALLYME_CRATES_ALLOW_ALREADY_PUBLISHED === "1";
  ...
  for (let attempt = 1; attempt <= 12; attempt += 1) {
    ...
    if (combined.includes("already uploaded") || combined.includes("already exists")) {
      if (allowAlreadyPublished) {
        console.log(
          `${pkg.name} ${pkg.version} is already published; continuing because REALLYME_CRATES_ALLOW_ALREADY_PUBLISHED=1.`,
        );
        return;
      }

      console.error(
        `${pkg.name} ${pkg.version} is already published. Refusing to continue because the script cannot prove the registry tarball matches this tree; set REALLYME_CRATES_ALLOW_ALREADY_PUBLISHED=1 only after manual verification.`,
      );
