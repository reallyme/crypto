...
protobuf/enums, package algorithm identifiers, typed error taxonomy, and shared
positive and negative conformance vectors. The Rust implementation is the
reference implementation and the shared implementation for selected primitives;
Swift, Kotlin, and TypeScript facades are first-class APIs that may route to
approved native providers only when the same contract is proven.
...
| AES-256-KW (RFC 3394) | ReallyMe Rust C ABI | BouncyCastle | ReallyMe WASM/Rust | Deterministic key-wrap; byte-identical regardless of provider. Kotlin routes through the bundled BouncyCastle provider to avoid Android provider-slot ambiguity. |
...
2. BouncyCastle for the 0.2 line when JVM and Android providers would otherwise
   diverge.
3. ReallyMe Rust C ABI or JNI, only through explicit provider-aware APIs.
4. Typed `unsupportedAlgorithm`.
...
WebCrypto is not part of the 0.2 line because it is async-only and would force
an API shape change. The TypeScript package keeps hand-written validation for
enum membership, byte lengths, buffer shape, and provider result shape.
...
- Swift, Kotlin/JVM, Kotlin/Android, and TypeScript lane hierarchies must remain
  present;
- TypeScript must remain sync-first and WebCrypto-free for the 0.2 line;
...
conformance vectors, and unsupported-provider coverage together.
...
TypeScript remains sync-first for the 0.2 line; WebCrypto is not a package
provider because it would force async facade signatures. Kotlin/JVM and
Kotlin/Android stay separate even when the 0.2 line routes both through the
