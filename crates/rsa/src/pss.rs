...
pub(crate) fn recover_encoded_message(
    ...
    if recovered.len() > key_size {
        // `crypto-bigint` rounds precision to its internal limb width. Reject
        // any non-zero precision padding before normalizing to the RSA modulus
        // length; otherwise a value wider than the modulus could be accepted.
        let prefix_len = recovered
            .len()
