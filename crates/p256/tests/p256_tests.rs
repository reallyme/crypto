...
use crypto_p256::{
    compress_p256, decompress_p256, derive_p256_shared_secret, generate_p256_keypair,
    generate_p256_keypair_from_secret_key, sign_p256_der_prehash, verify_p256_der_prehash,
    P256_SIGNATURE_DER_MAX_LEN,
};
...
fn key_sizes_are_correct() -> Result<(), CryptoError> {
    ...
    assert_eq!(P256_SIGNATURE_DER_MAX_LEN, 72);
    Ok(())
}
...
fn sign_and_verify_roundtrip() -> Result<(), CryptoError> {
    ...
    assert!(sig.len() <= P256_SIGNATURE_DER_MAX_LEN);
...
fn compression_helpers_reject_wrong_sec1_shapes() -> Result<(), CryptoError> {
    let (compressed, _secret_key) = generate_p256_keypair()?;
    let uncompressed = decompress_p256(&compressed)?;

    assert!(compress_p256(&compressed).is_err());
    assert!(decompress_p256(&uncompressed).is_err());

    let mut wrong_compressed_prefix = compressed;
    wrong_compressed_prefix[0] = 0x04;
    assert!(decompress_p256(&wrong_compressed_prefix).is_err());

    let mut wrong_uncompressed_prefix = uncompressed;
    wrong_uncompressed_prefix[0] = 0x02;
    assert!(compress_p256(&wrong_uncompressed_prefix).is_err());
    Ok(())
}
