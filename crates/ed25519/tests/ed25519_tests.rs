...
#[test]
fn expanded_private_key_is_rejected() -> Result<(), CryptoError> {
    let (pubkey, privkey) = generate_ed25519_keypair().into_test_result()?;
    ...
    let err = sign_ed25519(&expanded, b"expanded key must be rejected");
    assert!(matches!(err, Err(CryptoError::InvalidKey)));
    Ok(())
}
