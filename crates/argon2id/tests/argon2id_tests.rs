...
use crypto_argon2id::{
    ...
    DeriveKeyRequest, ARGON2ID_DERIVED_KEY_LENGTH, ARGON2ID_SECRET_MAX_LENGTH, ARGON2ID_V1_LANES,
...
fn caller_owned_zeroizing_memory_matches_upstream_argon2_output() -> Result<(), argon2::Error> {
    ...
    let mut expected = [0u8; ARGON2ID_DERIVED_KEY_LENGTH];
    argon2.hash_password_into(b"passphrase", &[7u8; 16], &mut expected)?;

    let mut blocks = Zeroizing::new(vec![argon2::Block::default(); block_count]);
    let mut actual = [0u8; ARGON2ID_DERIVED_KEY_LENGTH];
...
fn accepts_large_secret_input() {
    let profile = Argon2Profile::V1;
    let large_secret = vec![0x5a_u8; ARGON2ID_SECRET_MAX_LENGTH];
...
#[test]
fn rejects_secret_input_above_boundary_limit() {
    let oversized_length = match ARGON2ID_SECRET_MAX_LENGTH.checked_add(1) {
        Some(value) => value,
        None => return,
    };
    let oversized_secret = vec![0x5a_u8; oversized_length];
    let result = Argon2Secret::from_slice(&oversized_secret, Argon2Profile::V1);
    assert!(matches!(
        result,
        Err(CryptoError::Kdf {
            kind: KdfFailureKind::InvalidSecretLength,
