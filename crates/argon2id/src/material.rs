...
impl Argon2Secret {
    ...
    /// empty or exceeds [`ARGON2ID_SECRET_MAX_LENGTH`]. The `profile` is used
    /// only to tag any resulting error.
...
///
/// Salts can be stable account-correlating identifiers even when they are not
/// cryptographic secrets. This owner therefore redacts them by construction
/// and clears its allocation on drop.
#[derive(Zeroize, ZeroizeOnDrop)]
