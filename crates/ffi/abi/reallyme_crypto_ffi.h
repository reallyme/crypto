...
#define RM_CRYPTO_PROTOBUF_MAX_REQUEST_LEN         1048576
#define RM_CRYPTO_PROTO_JSON_MAX_REQUEST_LEN       1572864
#define RM_CRYPTO_OPERATION_RESPONSE_MAX_LEN       1048608
 ...
 * - Overlap checks include produced-length/status pointers and are enforced
 *   centrally for every input range registered by an exported operation.
 * - Callers must discard every output from a call that returns a status other
 *   than RM_CRYPTO_OK. Multi-output operations may have written an earlier
 *   output before a later write or unexpected internal failure is reported.
 *
 * Only rm_crypto_process_operation_response and
 * rm_crypto_process_operation_response_json define probe/fill sizing:
 * they write the required length to len_out on RM_CRYPTO_BUFFER_TOO_SMALL
 * when output is too small. A probe executes the complete operation; a later
 * fill call executes it again. Randomized responses can therefore differ, and
 * expensive operations pay their cost twice. Prefer allocating
 * RM_CRYPTO_OPERATION_RESPONSE_MAX_LEN when single execution is required.
 * Other variable-length scalar helpers do not define probe semantics; their
 * produced length outputs are meaningful only after RM_CRYPTO_OK unless the
 * function-specific comment says otherwise.
 */

/*
 * Operation response functions return binary CryptoOperationResponse bytes.
 * The generated response oneof carries either CryptoOperationResult or
 * CryptoError. The C status reports only pointer, buffer-sizing, or internal
 * ABI failure.
 */
rm_crypto_status_t rm_crypto_process_operation_response(
...
rm_crypto_status_t rm_crypto_process_operation_response_json(
...
#define RM_CRYPTO_AES128_KW_KEK_LEN         16
#define RM_CRYPTO_AES192_KW_KEK_LEN         24
...
#define RM_CRYPTO_KMAC256_MIN_KEY_LEN       32
#define RM_CRYPTO_KMAC256_MAX_KEY_LEN       4096
#define RM_CRYPTO_KMAC256_MAX_CONTEXT_LEN   65536
#define RM_CRYPTO_KMAC256_MAX_CUSTOMIZATION_LEN 4096
#define RM_CRYPTO_KMAC256_MAX_OUTPUT_LEN    65536
...
#define RM_CRYPTO_CHACHA20_POLY1305_KEY_LEN 32
#define RM_CRYPTO_CHACHA20_POLY1305_NONCE_LEN 12
#define RM_CRYPTO_XCHACHA20_POLY1305_NONCE_LEN 24
#define RM_CRYPTO_CHACHA20_POLY1305_TAG_LEN 16
...
#define RM_CRYPTO_PBKDF2_ITERATIONS_MIN     100000
#define RM_CRYPTO_PBKDF2_ITERATIONS_MAX     10000000
...
/* Ed25519 signing accepts only a 32-byte seed. Expanded 64-byte secret keys
 * are rejected rather than partially trusted or silently normalized. */
...
/*
 * Scalar AEAD and HPKE open functions intentionally collapse authentication/tag failures to RM_CRYPTO_AUTHENTICATION_FAILED.
 * Generated CryptoOperationResponse lanes preserve the typed
 * primitive/provider/backend classification without exposing backend exception
 * text.
 */
...
rm_crypto_status_t rm_crypto_aes128_kw_wrap_key(
    const uint8_t* kek,
    size_t kek_len,
...
rm_crypto_status_t rm_crypto_aes128_kw_unwrap_key(
    const uint8_t* kek,
    size_t kek_len,
...
rm_crypto_status_t rm_crypto_aes192_kw_wrap_key(
    const uint8_t* kek,
    size_t kek_len,
    const uint8_t* key_data,
    size_t key_data_len,
...
rm_crypto_status_t rm_crypto_aes192_kw_unwrap_key(
    const uint8_t* kek,
    size_t kek_len,
    const uint8_t* wrapped,
    size_t wrapped_len,
...
rm_crypto_status_t rm_crypto_kmac256_derive(
    const uint8_t* key,
    size_t key_len,
...
rm_crypto_status_t rm_crypto_chacha20_poly1305_encrypt(
    const uint8_t* key,
    size_t key_len,
    const uint8_t* nonce,
    size_t nonce_len,
...
rm_crypto_status_t rm_crypto_chacha20_poly1305_decrypt(
    const uint8_t* key,
    size_t key_len,
    const uint8_t* nonce,
    size_t nonce_len,
...
rm_crypto_status_t rm_crypto_xchacha20_poly1305_encrypt(
    const uint8_t* key,
    size_t key_len,
    const uint8_t* nonce,
    size_t nonce_len,
...
rm_crypto_status_t rm_crypto_xchacha20_poly1305_decrypt(
    const uint8_t* key,
    size_t key_len,
    const uint8_t* nonce,
    size_t nonce_len,
