...
use reallyme_crypto::hkdf::{DeriveRequest, HkdfInfo, HkdfInputKeyMaterial, HkdfSalt, HkdfSuite};
use zeroize::Zeroize;
...
fn derive_to_output<const N: usize>(
    ...
    let mut output = match reallyme_crypto::operations::kdf::derive_hkdf::<N>(request) {
        Ok(value) => value,
...
pub unsafe extern "C" fn rm_crypto_hkdf_derive(
    ...
    ffi_guard(|| {
        ...
        match output_out_len {
            16 | 24 | 32 | 48 | 64 => {}
            _ => return CRYPTO_INVALID_ARGUMENT,
        }
        // SAFETY: This preflight validates the output pointer before HKDF
        // reads inputs or derives secret output. The final write rechecks
        // overlap against the registered input ranges.
        let output_status = unsafe { write_fixed(output_out, output_out_len, &[]) };
