...
/// Length in bytes of the ML-DSA-87 secret key seed.
...
/// Validate that `pk` has the ML-DSA-87 public-key length, returning it
/// unchanged or [`CryptoError::InvalidKey`] otherwise.
pub fn assert_public_key(pk: &[u8]) -> Result<&[u8], CryptoError> {
    if pk.len() == ML_DSA_87_PUBLIC_KEY_LEN {
        Ok(pk)
    } else {
...
/// Encode an ML-DSA-87 public key as an owned `Vec` after validating its length.
pub fn encode_public_key(pk: &[u8]) -> Result<Vec<u8>, CryptoError> {
    Ok(assert_public_key(pk)?.to_vec())
}

/// Decode an ML-DSA-87 public key as an owned `Vec` after validating its length.
