...
final class RustCryptoFfi {
    ...
    private let xWing1024DerivePublicKeyFn: DerivePublicKeyFunction
    private let xWing1024EncapsulateDerandFn: EncapsulateDerandFunction
    private let xWing1024DecapsulateFn: DecapsulateFunction
    ...
    init() throws {
        ...
        xWing1024DerivePublicKeyFn = try Self.loadSymbol("rm_crypto_x_wing_1024_generate_keypair_derand", from: loadedHandle)
        xWing1024EncapsulateDerandFn = try Self.loadSymbol("rm_crypto_x_wing_1024_encapsulate_derand", from: loadedHandle)
        xWing1024DecapsulateFn = try Self.loadSymbol("rm_crypto_x_wing_1024_decapsulate", from: loadedHandle)
    }
    ...
    func aes256KwWrap(kek: [UInt8], keyData: [UInt8]) throws -> Data {
        ...
        return try callAesKw(aes256KwWrapKey, kek: kek, input: keyData, outputLength: outputLength)
    }
    ...
    func aes256KwUnwrap(kek: [UInt8], wrappedKey: [UInt8]) throws -> Data {
        ...
        return try callAesKw(aes256KwUnwrapKey, kek: kek, input: wrappedKey, outputLength: wrappedKey.count - 8)
    }
    ...
    func xWing1024PublicKey(secretKey: [UInt8]) throws -> Data {
        try derivePublicKey(xWing1024DerivePublicKeyFn, secretKey: secretKey, publicKeyLength: 1_600)
    }
    ...
    func xWing1024EncapsulateDerand(publicKey: [UInt8], seed: [UInt8]) throws -> (Data, Data) {
        try encapsulateDerand(
            xWing1024EncapsulateDerandFn,
            publicKey: publicKey,
            seed: seed,
    ...
    func xWing1024Decapsulate(ciphertext: [UInt8], secretKey: [UInt8]) throws -> Data {
        try decapsulateSharedSecret(xWing1024DecapsulateFn, ciphertext: ciphertext, secretKey: secretKey)
    }
    ...
    private static func buildLibrary(at repositoryRoot: URL) throws {
        ...
        process.arguments = ["cargo", "build", "-p", "crypto-ffi"]
