...
final class VectorConformanceTests: XCTestCase {
    ...
    func testRustFfiAes256KwVector() throws {
        let vector: Aes256KwVector = try loadVector("aes256kw.json")
        ...
        XCTAssertEqual(vector.alg, "AES-256-KW")
        XCTAssertEqual(try ffi.aes256KwWrap(kek: kek, keyData: keyData), wrappedKey)
        XCTAssertEqual(try ffi.aes256KwUnwrap(kek: kek, wrappedKey: [UInt8](wrappedKey)), Data(keyData))

        var tampered = [UInt8](wrappedKey)
        tampered[0] ^= 0x01
        XCTAssertThrowsError(try ffi.aes256KwUnwrap(kek: kek, wrappedKey: tampered))
    }
    ...
    func testRustFfiXWingKnownAnswer() throws {
        ...
        try assertXWingKnownAnswer(
            ...
            publicKeyLength: 1_600,
            ciphertextLength: 1_600,
            derivePublicKey: ffi.xWing1024PublicKey,
            encapsulateDerand: ffi.xWing1024EncapsulateDerand,
            decapsulate: ffi.xWing1024Decapsulate
        )
    }
    ...
    private func validateXWingShape() throws {
        ...
        try validateXWingCase(vectors.xWing1024, publicKeyLength: 1_600, ciphertextLength: 1_600)
    }
    ...
    private func validateAes256KwShape() throws {
        let vector: Aes256KwVector = try loadVector("aes256kw.json")

        XCTAssertEqual(vector.alg, "AES-256-KW")
        XCTAssertEqual(try Data(base64Url: vector.kek).count, 32)
        XCTAssertEqual(try Data(base64Url: vector.keyData).count, 32)
        XCTAssertEqual(try Data(base64Url: vector.wrappedKey).count, 40)
    }
    ...
    private func validateCodecShape() throws {
        let vector: CodecVector = try loadVector("codecs.json")
        XCTAssertFalse(try Data(base64Url: vector.raw).isEmpty)
        XCTAssertFalse(vector.base64url.isEmpty)
        XCTAssertTrue(vector.multibaseBase64url.hasPrefix("u"))
        XCTAssertTrue(vector.multibaseBase58btc.hasPrefix("z"))
        XCTAssertFalse(try Data(base64Url: vector.dagCbor).isEmpty)
        XCTAssertFalse(vector.dagCborCid.isEmpty)
        XCTAssertFalse(vector.multikey.isEmpty)
    }
    ...
    private func validateJwkShape() throws {
        let vectors: JwkVectors = try loadVector("jwk.json")
        XCTAssertEqual(vectors.vectors.count, 13)
        for vector in vectors.vectors {
            let publicKey = try Data(base64Url: vector.publicKey)
            XCTAssertEqual(publicKey.count, vector.publicKeyLength)
...
private struct Aes256KwVector: Decodable {
    let alg: String
    let kek: String
    let keyData: String
...
private struct CodecVector: Decodable {
    ...
    let multibaseBase64url: String
    let multibaseBase58btc: String
    let dagCbor: String
    ...
    enum CodingKeys: String, CodingKey {
        case raw
        case base64url
        case multibaseBase64url = "multibase_base64url"
        case multibaseBase58btc = "multibase_base58btc"
        case dagCbor = "dag_cbor"
        case dagCborCid = "dag_cbor_cid"
        case multikey
    }
}
...
private enum SwiftJwk {
    ...
    private static func spec(for alg: String) throws -> SwiftJwkSpec {
        ...
        case "X-Wing-1024":
            return SwiftJwkSpec(alg: alg, crv: alg, kty: "AKP", keyUse: "enc", publicKeyLength: 1_600)
        default:
            throw VectorError.invalidField
        }
    }
