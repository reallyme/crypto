...
use crate::support::{field_array, load, VectorTestError};
...
fn protobuf_algorithm_enum_numbers_are_stable() -> Result<(), VectorTestError> {
    let proto = read_repo_file("crates/proto/crypto/proto/reallyme/crypto/v1/crypto.proto")?;
...
fn protobuf_byte_helpers_do_not_use_plain_proto_names() -> Result<(), VectorTestError> {
    ...
    assert_proto_result_suffix_is_reserved_for_status_envelope(
        "TypeScript",
        &ts,
        &["ReallyMeCryptoProtoResult", "cryptoProtoResult"],
    );
    assert_proto_result_suffix_is_reserved_for_status_envelope(
        "Kotlin",
        &kotlin,
...
fn assert_proto_result_suffix_is_reserved_for_status_envelope(
    lane: &str,
    source: &str,
    allowed_names: &[&str],
) {
    for name in source.match_indices("ProtoResult") {
        let start = source[..name.0]
            .rfind(|ch: char| !(ch.is_ascii_alphanumeric() || ch == '_'))
            .map_or(0, |index| index + 1);
        let end = source[name.0..]
            .find(|ch: char| !(ch.is_ascii_alphanumeric() || ch == '_'))
            .map_or(source.len(), |index| name.0 + index);
        let identifier = &source[start..end];
        assert!(
            allowed_names.contains(&identifier),
            "{lane} crypto proto adapters must reserve ProtoResult for the status-plus-bytes \
             success/error envelope; decoded multi-field values must use ProtoValue"
        );
    }
}
...
fn protobuf_operation_contract_captures_public_facade_io() -> Result<(), VectorTestError> {
    ...
    let actual_operations = collect_ts_crypto_facade_methods(&facade);
...
fn typescript_contract_is_sync_and_webcrypto_free() -> Result<(), VectorTestError> {
    ...
    assert!(contract.contains("The TypeScript facade is synchronous for the 0.2 line."));
    assert!(contract.contains("WebCrypto is not part of the 0.2 facade"));
    assert!(policy.contains("WebCrypto is not part of the 0.2 line"));
...
fn canonical_contract_is_documented_and_not_rust_api_only() -> Result<(), VectorTestError> {
    ...
    let root_rustdoc = read_repo_file("src/lib.rs")?;
