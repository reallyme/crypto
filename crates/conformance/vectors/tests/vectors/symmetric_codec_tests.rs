...
use crate::support::{
    b64u_to_bytes, field_array, field_string, load, object_field, VectorTestError,
};
...
#[test]
fn codec_vectors_round_trip() -> Result<(), VectorTestError> {
    let v = load("codecs.json")?;
    let raw = b64u_to_bytes(field_string(&v, "raw")?)?;
    let base64url = field_string(&v, "base64url")?;
    let multibase_base64url = field_string(&v, "multibase_base64url")?;
    let multibase_base58btc = field_string(&v, "multibase_base58btc")?;
    let dag_cbor = b64u_to_bytes(field_string(&v, "dag_cbor")?)?;
    let dag_cbor_cid = field_string(&v, "dag_cbor_cid")?;
    let multikey = field_string(&v, "multikey")?;

    assert_eq!(
        base64url_to_bytes(base64url).map_err(|_| VectorTestError::DecodeBase64Url)?,
        raw
    );
    assert_eq!(
        multibase_to_bytes(multibase_base64url).map_err(|_| VectorTestError::MultibaseParse)?,
        raw
    );
    assert_eq!(
        multibase_to_bytes(multibase_base58btc).map_err(|_| VectorTestError::MultibaseParse)?,
        raw
    );
    let (cid_valid, _, _) = verify_dag_cbor_cid(dag_cbor_cid, &dag_cbor);
    assert!(cid_valid);

    let decoded = decode_dag_cbor(&dag_cbor).map_err(|_| VectorTestError::CborDecode)?;
    assert!(matches!(decoded, CborValue::Map(_)));

    let parsed = parse_multikey(multikey).map_err(|_| VectorTestError::MultikeyParse)?;
...
fn assert_codec_prefix_vector(
    ...
    let entry = entries
        .iter()
        .find(|entry| {
            field_string(entry, "name")
                .map(|entry_name| entry_name == name)
                .unwrap_or(false)
        })
        .ok_or(VectorTestError::InvalidField)?;
    let prefix = b64u_to_bytes(field_string(entry, "prefix")?)?;
    assert_eq!(prefix, expected_prefix);
    Ok(())
}

#[test]
fn codec_prefix_vectors_match_multicodec_table() -> Result<(), VectorTestError> {
    let v = load("codecs.json")?;
    let prefixes = field_array(&v, "multicodec_prefixes")?;

    assert_codec_prefix_vector(prefixes, "sha2-256", &[0x12])?;
    assert_codec_prefix_vector(prefixes, "sha2-384", &[0x20])?;
    assert_codec_prefix_vector(prefixes, "sha2-512", &[0x13])?;
    assert_codec_prefix_vector(prefixes, "sha3-224", &[0x17])?;
    assert_codec_prefix_vector(prefixes, "sha3-256", &[0x16])?;
    assert_codec_prefix_vector(prefixes, "sha3-384", &[0x15])?;
    assert_codec_prefix_vector(prefixes, "sha3-512", &[0x14])?;
    assert_codec_prefix_vector(prefixes, "p384-pub", &[0x81, 0x24])?;
    assert_codec_prefix_vector(prefixes, "p521-pub", &[0x82, 0x24])?;
    assert_codec_prefix_vector(prefixes, "ed448-pub", &[0x83, 0x24])?;
    assert_codec_prefix_vector(prefixes, "rsa-pub", &[0x85, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mlkem-512-pub", &[0x8b, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mlkem-768-pub", &[0x8c, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mlkem-1024-pub", &[0x8d, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mldsa-44-pub", &[0x90, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mldsa-65-pub", &[0x91, 0x24])?;
    assert_codec_prefix_vector(prefixes, "mldsa-87-pub", &[0x92, 0x24])?;
    assert_codec_prefix_vector(prefixes, "p256-priv", &[0x86, 0x26])?;
    assert_codec_prefix_vector(prefixes, "p384-priv", &[0x87, 0x26])?;
    assert_codec_prefix_vector(prefixes, "p521-priv", &[0x88, 0x26])?;
    assert_codec_prefix_vector(prefixes, "mlkem-512-priv", &[0x93, 0x26])?;
    assert_codec_prefix_vector(prefixes, "mlkem-768-priv", &[0x94, 0x26])?;
    assert_codec_prefix_vector(prefixes, "mlkem-1024-priv", &[0x95, 0x26])?;
    assert_codec_prefix_vector(prefixes, "aes-gcm-256", &[0x80, 0x40])?;
    assert_codec_prefix_vector(prefixes, "chacha20-poly1305", &[0x80, 0xc0, 0x02])?;
