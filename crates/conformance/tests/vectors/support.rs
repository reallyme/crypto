...
pub(crate) fn read_repo_file(path: &str) -> Result<String, VectorTestError> {
    let package = package_dir();
    let repo_root = package
        .parent()
...
pub(crate) fn external_vectors_dir() -> Result<PathBuf, VectorTestError> {
    Ok(vectors_dir()?.join("external"))
}

pub(crate) fn read_external_bytes(name: &str) -> Result<Vec<u8>, VectorTestError> {
    fs::read(external_vectors_dir()?.join(name)).map_err(|_| VectorTestError::ReadVector)
}
...
pub(crate) fn load_external(name: &str) -> Result<Value, VectorTestError> {
    read_json(external_vectors_dir()?.join(name))
}
