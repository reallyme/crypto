...
#[test]
fn rejects_redundant_positive_integer_padding() {
    let der = hex!(
        "3027
         0222
...
#[test]
fn rejects_noncanonical_der_long_form_lengths() {
...
#[test]
fn rejects_zero_and_out_of_range_secp256k1_scalars() {
    let one = scalar_with_last(0x01);
    let order = hex!("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141");
    for raw in [
        raw_signature_from_scalars([0u8; 32], one),
        raw_signature_from_scalars(one, [0u8; 32]),
    ...
    let order_r =
        hex!("3026022100fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141020101");
    let order_s =
        hex!("3026020101022100fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141");
    for der in [
        zero_r.as_slice(),
        zero_s.as_slice(),
        order_r.as_slice(),
    ...
    #[cfg(all(feature = "native", not(all(feature = "wasm", target_arch = "wasm32"))))]
    {
        assert!(Secp256k1Signature::from_der(&order_r).is_err());
        assert!(Secp256k1Signature::from_der(&order_s).is_err());
    }
}
