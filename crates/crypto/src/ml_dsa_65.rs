...
//! ML-DSA-65 root facade routed through the signature operation layer.
...
pub use crypto_ml_dsa_65::{
    decode_public_key, encode_public_key, ML_DSA_65_PUBLIC_KEY_LEN, ML_DSA_65_SECRET_SEED_LEN,
    ML_DSA_65_SIGNATURE_LEN,
};

/// Generate an ML-DSA-65 keypair through the signature operation owner.
pub fn generate_ml_dsa_65_keypair() -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), CryptoError> {
    crate::operations::signature::generate_key_pair(Algorithm::MlDsa65)
...
/// Reconstruct an ML-DSA-65 keypair from a 32-byte FIPS 204 seed.
pub fn generate_ml_dsa_65_keypair_from_seed(
    seed: &[u8; ML_DSA_65_SECRET_SEED_LEN],
) -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), CryptoError> {
    crate::operations::signature::derive_key_pair(Algorithm::MlDsa65, seed)
        .map(|key_pair| (key_pair.public_key, key_pair.secret_key))
        .map_err(|error| {
            crypto_error_from_operation_error(SignatureOperation::KeyManagement, error)
        })
...
/// Sign `message` with an ML-DSA-65 seed.
pub fn sign_ml_dsa_65(secret_seed: &[u8], message: &[u8]) -> Result<Vec<u8>, CryptoError> {
    crate::operations::signature::sign(Algorithm::MlDsa65, secret_seed, message)
...
pub fn verify_ml_dsa_65(
    ...
    crate::operations::signature::verify(Algorithm::MlDsa65, public_key, message, signature)
