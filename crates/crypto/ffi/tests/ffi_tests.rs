...
use crypto_proto::generated::proto::reallyme::crypto::v1::{
    ...
    CryptoAlgorithmIdentifier, CryptoErrorReason, CryptoHashRequest, CryptoHashResult,
    HashAlgorithm,
};
use crypto_proto::wire::{CryptoWireError, MAX_CRYPTO_PROTO_MESSAGE_BYTES};
use reallyme_crypto::proto_process::{
    OP_AEAD_OPEN, OP_AEAD_SEAL, OP_BIP340_SCHNORR_SIGN, OP_HASH, OP_HKDF_DERIVE, OP_HPKE_OPEN,
    OP_HPKE_SEAL, OP_JWA_CONCAT_KDF_SHA256_DERIVE, OP_KDF_DERIVE_KEY, OP_KEM_DECAPSULATE,
    OP_KEM_ENCAPSULATE, OP_KEM_GENERATE_KEY_PAIR, OP_KEY_AGREEMENT_DERIVE_KEY_PAIR,
    OP_KEY_AGREEMENT_DERIVE_SHARED_SECRET, OP_KEY_UNWRAP, OP_KEY_WRAP, OP_MAC_AUTHENTICATE,
    OP_MAC_VERIFY, OP_RSA_VERIFY, OP_SIGNATURE_DERIVE_KEY_PAIR, OP_SIGNATURE_GENERATE_KEY_PAIR,
    OP_SIGNATURE_SIGN, OP_SIGNATURE_VERIFY,
};

use buffa::{EnumValue, Message, MessageField};

#[test]
fn ffi_header_operation_ids_match_the_generated_proto_contract() {
    let header = include_str!("../abi/reallyme_crypto_ffi.h");
    for (name, value) in [
        ("RM_CRYPTO_PROTO_OP_HASH", OP_HASH),
        ("RM_CRYPTO_PROTO_OP_AEAD_SEAL", OP_AEAD_SEAL),
        ("RM_CRYPTO_PROTO_OP_AEAD_OPEN", OP_AEAD_OPEN),
        ("RM_CRYPTO_PROTO_OP_MAC_AUTHENTICATE", OP_MAC_AUTHENTICATE),
        ("RM_CRYPTO_PROTO_OP_MAC_VERIFY", OP_MAC_VERIFY),
        (
            "RM_CRYPTO_PROTO_OP_SIGNATURE_GENERATE_KEY_PAIR",
            OP_SIGNATURE_GENERATE_KEY_PAIR,
        ),
        (
            "RM_CRYPTO_PROTO_OP_SIGNATURE_DERIVE_KEY_PAIR",
            OP_SIGNATURE_DERIVE_KEY_PAIR,
        ),
        ("RM_CRYPTO_PROTO_OP_SIGNATURE_SIGN", OP_SIGNATURE_SIGN),
        ("RM_CRYPTO_PROTO_OP_SIGNATURE_VERIFY", OP_SIGNATURE_VERIFY),
        (
            "RM_CRYPTO_PROTO_OP_BIP340_SCHNORR_SIGN",
            OP_BIP340_SCHNORR_SIGN,
        ),
        ("RM_CRYPTO_PROTO_OP_RSA_VERIFY", OP_RSA_VERIFY),
        (
            "RM_CRYPTO_PROTO_OP_KEY_AGREEMENT_DERIVE_SHARED_SECRET",
            OP_KEY_AGREEMENT_DERIVE_SHARED_SECRET,
        ),
        (
            "RM_CRYPTO_PROTO_OP_KEY_AGREEMENT_DERIVE_KEY_PAIR",
            OP_KEY_AGREEMENT_DERIVE_KEY_PAIR,
        ),
        (
            "RM_CRYPTO_PROTO_OP_KEM_GENERATE_KEY_PAIR",
            OP_KEM_GENERATE_KEY_PAIR,
        ),
        ("RM_CRYPTO_PROTO_OP_KEM_ENCAPSULATE", OP_KEM_ENCAPSULATE),
        ("RM_CRYPTO_PROTO_OP_KEM_DECAPSULATE", OP_KEM_DECAPSULATE),
        ("RM_CRYPTO_PROTO_OP_HKDF_DERIVE", OP_HKDF_DERIVE),
        ("RM_CRYPTO_PROTO_OP_KDF_DERIVE_KEY", OP_KDF_DERIVE_KEY),
        (
            "RM_CRYPTO_PROTO_OP_JWA_CONCAT_KDF_SHA256_DERIVE",
            OP_JWA_CONCAT_KDF_SHA256_DERIVE,
        ),
        ("RM_CRYPTO_PROTO_OP_KEY_WRAP", OP_KEY_WRAP),
        ("RM_CRYPTO_PROTO_OP_KEY_UNWRAP", OP_KEY_UNWRAP),
        ("RM_CRYPTO_PROTO_OP_HPKE_SEAL", OP_HPKE_SEAL),
        ("RM_CRYPTO_PROTO_OP_HPKE_OPEN", OP_HPKE_OPEN),
    ] {
        let expected = format!("#define {name}");
        let line = header.lines().find(|line| line.starts_with(&expected));
        let value_text = value.to_string();
        assert!(
            matches!(line, Some(line) if line.split_whitespace().last() == Some(value_text.as_str()))
        );
    }
    assert!(header.contains("#define RM_CRYPTO_PROTO_MAX_REQUEST_LEN           1048576"));
}
...
#[test]
fn proto_process_ffi_returns_result_and_error_statuses() {
    ...
    let result_status = unsafe {
        proto_process::rm_crypto_process_proto(
            OP_HASH,
    ...
    let mut result_bytes = &output[..output_len];
    let result = CryptoHashResult::decode(&mut result_bytes).expect("hash result decodes");
    assert_eq!(result.digest.len(), sha2_256::SHA2_256_DIGEST_LEN);

    let malformed = [0xff_u8, 0xff, 0xff];
    let mut error_output = [0_u8; 128];
    let mut error_output_len = 0_usize;
    let error_status = unsafe {
        proto_process::rm_crypto_process_proto(
            OP_HASH,
            malformed.as_ptr(),
            malformed.len(),
            error_output.as_mut_ptr(),
            error_output.len(),
            &mut error_output_len,
        )
    };

    assert_eq!(error_status, status::CRYPTO_PROTO_ERROR);
    let error = CryptoWireError::decode(&error_output[..error_output_len])
        .expect("crypto error bytes decode");
    assert_eq!(
        error.reason(),
...
#[test]
fn proto_process_ffi_reports_required_length_for_short_output() {
    let request = CryptoHashRequest {
        algorithm: MessageField::some(CryptoAlgorithmIdentifier {
            algorithm: Some(ProtoAlgorithmBranch::Hash(EnumValue::from(
                HashAlgorithm::HASH_ALGORITHM_SHA2_256,
...
#[test]
fn proto_process_ffi_rejects_overlap_and_oversized_requests_before_decode() {
    let mut storage = [0xA5_u8; 128];
    let request_ptr = storage.as_ptr();
    let output_ptr = unsafe { storage.as_mut_ptr().add(8) };
    let mut output_len = 0_usize;

    let overlap_status = unsafe {
        proto_process::rm_crypto_process_proto(
            OP_HASH,
            request_ptr,
            16,
    ...
    assert_eq!(overlap_status, status::CRYPTO_INVALID_ARGUMENT);
    assert_eq!(storage, [0xA5_u8; 128]);
    ...
    let oversized_status = unsafe {
        proto_process::rm_crypto_process_proto(
            OP_HASH,
            NonNull::<u8>::dangling().as_ptr(),
            MAX_CRYPTO_PROTO_MESSAGE_BYTES + 1,
            storage.as_mut_ptr(),
            storage.len(),
            &mut output_len,
        )
    };
    assert_eq!(oversized_status, status::CRYPTO_INVALID_ARGUMENT);
    assert_eq!(output_len, 0);
}
