...
fn exported_ffi_symbol_names(source: &str) -> Vec<&str> {
    ...
    while let Some(relative_start) = source[search_from..].find(EXTERN_EXPORT_PREFIX) {
        ...
        let name_start = start
            .checked_add("pub unsafe extern \"C\" fn ".len())
            .expect("name start should not overflow");
        let name_end_relative = source[name_start..]
            .find('(')
            .expect("exported function should have a parameter list");
        let name_end = name_start
            .checked_add(name_end_relative)
            .expect("name end should not overflow");
        symbols.push(&source[name_start..name_end]);
