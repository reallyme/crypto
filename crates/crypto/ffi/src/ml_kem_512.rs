...
/// Length in bytes of an ML-KEM-512 public key (800).
pub const ML_KEM_512_PUBLIC_KEY_LEN: usize = 800;
/// Length in bytes of an ML-KEM-512 secret key as handled here (64).
pub const ML_KEM_512_SECRET_KEY_LEN: usize = 64;
/// Length in bytes of an ML-KEM-512 ciphertext (768).
pub const ML_KEM_512_CIPHERTEXT_LEN: usize = 768;
/// Length in bytes of an ML-KEM-512 shared secret (32).
pub const ML_KEM_512_SHARED_SECRET_LEN: usize = 32;
/// Length in bytes of ML-KEM deterministic encapsulation randomness.
pub const ML_KEM_512_ENCAPS_RANDOMNESS_LEN: usize = 32;

/// Generates an ML-KEM-512 keypair, writing the 800-byte public key to
/// `public_out` and the 64-byte secret key to `secret_out`.
///
/// # Safety
///
/// `public_out` must point to at least `public_out_len` writable bytes (at
/// least 800) and `secret_out` to at least `secret_out_len` writable bytes
...
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_generate_keypair(
    public_out: *mut u8,
    ...
    ffi_guard(|| {
        ...
        let (public, secret) =
            match crypto_ml_kem_512::generate_ml_kem_512_keypair().into_keypair_result() {
                Ok(value) => value,
...
/// Derives an ML-KEM-512 keypair from a caller-supplied 64-byte FIPS seed.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_generate_keypair_from_seed(
    secret_key: *const u8,
    ...
    ffi_guard(|| {
        ...
        let secret_key = match <&[u8; ML_KEM_512_SECRET_KEY_LEN]>::try_from(secret_key) {
            Ok(value) => value,
            Err(_) => return CRYPTO_INVALID_KEY,
        };
        let (public, secret) =
            match crypto_ml_kem_512::generate_ml_kem_512_keypair_from_seed(secret_key) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
        let status = unsafe { write_fixed(public_out, public_out_len, &public) };
...
/// Encapsulates to the ML-KEM-512 `public_key`, writing the 768-byte
...
/// `ciphertext_out_len` writable bytes (at least 768) and `shared_secret_out`
...
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_encapsulate(
    public_key: *const u8,
    ...
    ffi_guard(|| {
        ...
        let (ciphertext, shared_secret) =
            match crypto_ml_kem_512::ml_kem_512_encapsulate(public_key) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
...
/// Deterministically encapsulates to an ML-KEM-512 public key.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_encapsulate_derand(
    public_key: *const u8,
    public_key_len: usize,
    randomness: *const u8,
    ...
    ffi_guard(|| {
        ...
        if randomness.len() != ML_KEM_512_ENCAPS_RANDOMNESS_LEN {
            return CRYPTO_INVALID_KEY;
        }
        let (ciphertext, shared_secret) =
            match crypto_ml_kem_512::ml_kem_512_encapsulate_derand(public_key, randomness) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
        let status = unsafe { write_fixed(ciphertext_out, ciphertext_out_len, &ciphertext) };
...
/// Decapsulates an ML-KEM-512 `ciphertext` with `secret_key`, writing the
/// recovered 32-byte shared secret to `shared_secret_out`.
///
/// # Safety
///
/// `ciphertext` must be valid for `ciphertext_len` bytes (must be 768) and
...
/// zeroize it.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_decapsulate(
    ciphertext: *const u8,
    ...
    ffi_guard(|| {
        ...
        match crypto_ml_kem_512::ml_kem_512_decapsulate(ciphertext, secret_key) {
...
/// Validates a 800-byte ML-KEM-512 `public_key` and copies its canonical
/// encoding to `out`.
///
/// # Safety
///
/// `public_key` must be valid for `public_key_len` bytes (must be 800; may be
/// null only when the length is `0`). `out` must be non-null and point to at
/// least `out_len` writable bytes (at least 800). Returns [`CryptoStatus`]
/// via the return value: [`CRYPTO_OK`] on success, or [`CRYPTO_INVALID_KEY`]
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_encode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
...
/// Validates a 800-byte encoded ML-KEM-512 `public_key` and copies the
...
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_512_decode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
