...
//! status code. This makes the no-unwind guarantee a property of the code
//! itself: it no longer depends on `[profile.release] panic = "abort"` and so
//! also holds for debug builds and for downstream consumers that static-link
...
/// C ABI surface for AES-256-KW key wrapping.
pub mod aes_kw;
...
pub mod pointer;
/// Codec-style protobuf operation entrypoint.
