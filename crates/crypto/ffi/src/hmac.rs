...
use crypto_hmac::{authenticate, verify, HmacKey};
...
pub const HMAC_SHA256_TAG_LEN: usize = crypto_hmac::HMAC_SHA256_TAG_LENGTH;
/// Length in bytes of an HMAC-SHA-512 tag (64).
pub const HMAC_SHA512_TAG_LEN: usize = crypto_hmac::HMAC_SHA512_TAG_LENGTH;
...
fn status_from_error(error: CryptoError) -> CryptoStatus {
    match error {
        CryptoError::Mac {
...
pub unsafe extern "C" fn rm_crypto_hmac_authenticate(
    ...
    ffi_guard(|| {
        ...
        let key = match unsafe { read_slice(key, key_len) } {
            Ok(value) => match HmacKey::from_slice(value) {
                Ok(key) => key,
        ...
        let tag = match authenticate(suite, &key, message) {
            Ok(value) => value,
            Err(error) => return status_from_error(error),
        };

        unsafe { write_fixed(tag_out, tag_out_len, tag.as_bytes()) }
    })
}
...
pub unsafe extern "C" fn rm_crypto_hmac_verify(
    ...
    ffi_guard(|| {
        ...
        match verify(suite, &key, message, tag) {
