...
struct InputRangeRegistry {
    ranges: [InputRange; MAX_FFI_INPUT_RANGES_PER_CALL],
    count: usize,
}

impl InputRangeRegistry {
    const fn new() -> Self {
        Self {
            ranges: [InputRange::EMPTY; MAX_FFI_INPUT_RANGES_PER_CALL],
            count: 0,
        }
    }

    fn clear(&mut self) {
        self.ranges.fill(InputRange::EMPTY);
        self.count = 0;
    }
}

std::thread_local! {
    static INPUT_RANGES: RefCell<InputRangeRegistry> = const {
        RefCell::new(InputRangeRegistry::new())
    };
}

/// Per-call guard that clears raw input-range metadata after an FFI operation.
pub(crate) struct InputRangeCallGuard;

impl Drop for InputRangeCallGuard {
    fn drop(&mut self) {
        let _ = INPUT_RANGES.try_with(|registry| {
...
/// Starts raw input-range tracking for one exported FFI call.
pub(crate) fn begin_input_range_call() -> Result<InputRangeCallGuard, CryptoStatus> {
    INPUT_RANGES
        .try_with(|registry| {
    ...
    INPUT_RANGES
        .try_with(|registry| {
            let mut ranges = registry
                .try_borrow_mut()
                .map_err(|_| CRYPTO_INVALID_ARGUMENT)?;
            if ranges.count >= MAX_FFI_INPUT_RANGES_PER_CALL {
                return Err(CRYPTO_INVALID_ARGUMENT);
            }
            let index = ranges.count;
            ranges.ranges[index] = InputRange { ptr, len };
            ranges.count = index.checked_add(1).ok_or(CRYPTO_INVALID_ARGUMENT)?;
            Ok(())
        })
        .map_err(|_| CRYPTO_INVALID_ARGUMENT)?
}
...
fn validate_registered_inputs_against_output<T>(
    ...
    INPUT_RANGES
        .try_with(|registry| {
            ...
            for input in &ranges.ranges[..ranges.count] {
