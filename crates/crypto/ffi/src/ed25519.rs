...
use crate::status::{
    CryptoStatus, IntoKeypairResult, CRYPTO_INTERNAL_ERROR, CRYPTO_INVALID_KEY,
...
/// Length in bytes of an expanded Ed25519 secret key (64), also accepted by
...
pub unsafe extern "C" fn rm_crypto_ed25519_generate_keypair(
    ...
    ffi_guard(|| {
        ...
        let (public, secret) =
            match crypto_ed25519::generate_ed25519_keypair().into_keypair_result() {
                Ok(value) => value,
...
pub unsafe extern "C" fn rm_crypto_ed25519_generate_keypair_from_seed(
    ...
    ffi_guard(|| {
        ...
        let (public, secret) = crypto_ed25519::generate_ed25519_keypair_from_seed(seed);
        let status = unsafe { write_fixed(public_out, public_out_len, &public) };
        if status != CRYPTO_OK {
            return status;
        }
...
/// signature to `signature_out`. The secret must be 32 bytes (seed) or 64
/// bytes (expanded).
///
/// # Safety
///
...
pub unsafe extern "C" fn rm_crypto_ed25519_sign(
    ...
    ffi_guard(|| {
        ...
        if secret.len() != ED25519_SECRET_KEY_LEN && secret.len() != ED25519_EXPANDED_SECRET_KEY_LEN
        {
        ...
        match crypto_ed25519::sign_ed25519(secret, message) {
...
pub unsafe extern "C" fn rm_crypto_ed25519_verify(
    ...
    ffi_guard(|| {
        ...
        match crypto_ed25519::verify_ed25519(public_key, message, signature) {
            Ok(()) => CRYPTO_OK,
            Err(crypto_core::CryptoError::InvalidKey) => CRYPTO_INVALID_KEY,
...
pub unsafe extern "C" fn rm_crypto_ed25519_encode_public_key(
    ...
    ffi_guard(|| {
        ...
        match crypto_ed25519::encode_public_key(public_key) {
...
pub unsafe extern "C" fn rm_crypto_ed25519_decode_public_key(
    ...
    ffi_guard(|| {
        ...
        match crypto_ed25519::decode_public_key(public_key) {
            Ok(value) => unsafe { write_fixed(out, out_len, &value) },
            Err(_) => CRYPTO_INVALID_KEY,
        }
    })
