...
pub unsafe extern "C" fn rm_crypto_slh_dsa_sha2_128s_generate_keypair(
    ...
    ffi_guard(|| {
        ...
        let (public_key, secret_key) =
            match crypto_slh_dsa::generate_slh_dsa_sha2_128s_keypair().into_keypair_result() {
                Ok(value) => value,
                Err(_) => return CRYPTO_INTERNAL_ERROR,
            };
        let status = unsafe { write_fixed(public_out, public_out_len, &public_key) };
        if status != CRYPTO_OK {
            return status;
        }
        unsafe { write_fixed(secret_key_out, secret_key_out_len, &secret_key) }
    })
}
...
pub unsafe extern "C" fn rm_crypto_slh_dsa_sha2_128s_derive_keypair(
    ...
    ffi_guard(|| {
        ...
        let (public_key, secret_key) =
            match crypto_slh_dsa::derive_slh_dsa_sha2_128s_keypair(sk_seed, sk_prf, pk_seed)
                .into_keypair_result()
            {
                Ok(value) => value,
                Err(_) => return CRYPTO_INTERNAL_ERROR,
...
pub unsafe extern "C" fn rm_crypto_slh_dsa_sha2_128s_sign(
    ...
    ffi_guard(|| {
        ...
        match crypto_slh_dsa::sign_slh_dsa_sha2_128s(secret_key, message) {
...
pub unsafe extern "C" fn rm_crypto_slh_dsa_sha2_128s_verify(
    ...
    ffi_guard(|| {
        ...
        match crypto_slh_dsa::verify_slh_dsa_sha2_128s(public_key, message, signature) {
            Ok(()) => CRYPTO_OK,
