...
/// Maximum length in bytes of a DER-encoded P-256 ECDSA signature (80).
pub const P256_SIGNATURE_DER_MAX_LEN: usize = 80;
...
pub unsafe extern "C" fn rm_crypto_p256_generate_keypair(
    ...
    ffi_guard(|| {
        ...
        let (public, secret) = match crypto_p256::generate_p256_keypair().into_keypair_result() {
            Ok(value) => value,
...
pub unsafe extern "C" fn rm_crypto_p256_generate_keypair_from_secret_key(
    ...
    ffi_guard(|| {
        ...
        let (public, secret) = match crypto_p256::generate_p256_keypair_from_secret_key(secret_key)
        {
            Ok(value) => value,
            Err(_) => return CRYPTO_INVALID_KEY,
        };
...
/// Signs a pre-hashed `message` (message digest) with the P-256 `secret_key`,
/// writing the DER-encoded ECDSA signature to `signature_out` and its actual
/// length to `signature_len_out`.
///
/// # Safety
///
...
/// 80). `signature_len_out` must be a non-null writable `usize`. Returns
...
pub unsafe extern "C" fn rm_crypto_p256_sign_der_prehash(
    ...
    ffi_guard(|| {
        ...
        let signature = match crypto_p256::sign_p256_der_prehash(secret_key, message) {
            Ok(value) => value,
            Err(_) => return CRYPTO_INVALID_KEY,
        };
...
pub unsafe extern "C" fn rm_crypto_p256_verify_der_prehash(
    ...
    ffi_guard(|| {
        ...
        match crypto_p256::verify_p256_der_prehash(signature, message, public_key) {
            Ok(()) => CRYPTO_OK,
...
pub unsafe extern "C" fn rm_crypto_p256_derive_shared_secret(
    ...
    ffi_guard(|| {
        ...
        match crypto_p256::derive_p256_shared_secret(secret_key, public_key) {
...
pub unsafe extern "C" fn rm_crypto_p256_compress_public_key(
    ...
    ffi_guard(|| {
        ...
        match crypto_p256::compress_p256(public_key) {
...
pub unsafe extern "C" fn rm_crypto_p256_decompress_public_key(
    ...
    ffi_guard(|| {
        ...
        match crypto_p256::decompress_p256(public_key) {
            Ok(value) => unsafe { write_fixed(out, out_len, &value) },
            Err(_) => CRYPTO_INVALID_KEY,
        }
    })
