...
/// Length in bytes of an ML-DSA-65 public key (1952).
pub const ML_DSA_65_PUBLIC_KEY_LEN: usize = 1952;
/// Length in bytes of an ML-DSA-65 secret-key seed (32).
pub const ML_DSA_65_SECRET_SEED_LEN: usize = 32;
/// Length in bytes of an ML-DSA-65 signature (3309).
pub const ML_DSA_65_SIGNATURE_LEN: usize = 3309;

/// Generates an ML-DSA-65 keypair, writing the 1952-byte public key to
...
/// least 1952) and `secret_seed_out` to at least `secret_seed_out_len`
...
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_generate_keypair(
    public_out: *mut u8,
    ...
    ffi_guard(|| {
        ...
        let (public, secret) =
            match crypto_ml_dsa_65::generate_ml_dsa_65_keypair().into_keypair_result() {
                Ok(value) => value,
...
/// Derives an ML-DSA-65 keypair from a caller-supplied 32-byte seed.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_generate_keypair_from_seed(
    ...
    ffi_guard(|| {
        ...
        let secret_seed = match <&[u8; ML_DSA_65_SECRET_SEED_LEN]>::try_from(secret_seed) {
            Ok(value) => value,
            Err(_) => return CRYPTO_INVALID_KEY,
        };
        let (public, secret) =
            match crypto_ml_dsa_65::generate_ml_dsa_65_keypair_from_seed(secret_seed) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
        let status = unsafe { write_fixed(public_out, public_out_len, &public) };
...
/// Signs `message` with the ML-DSA-65 `secret_seed`, writing the 3309-byte
...
/// `signature_out_len` writable bytes (at least [`ML_DSA_65_SIGNATURE_LEN`],
/// 3309). Returns [`CryptoStatus`] via the return value: [`CRYPTO_OK`] on
/// success, or [`CRYPTO_INVALID_KEY`] for a bad seed length or key.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_sign(
    secret_seed: *const u8,
    secret_seed_len: usize,
    message: *const u8,
    message_len: usize,
    ...
    ffi_guard(|| {
        ...
        if secret_seed.len() != ML_DSA_65_SECRET_SEED_LEN {
        ...
        match crypto_ml_dsa_65::sign_ml_dsa_65(secret_seed, message) {
...
/// Verifies an ML-DSA-65 `signature` over `message` against `public_key`,
/// writing `1` (valid) or `0` (invalid) to `valid_out`.
///
/// # Safety
///
/// `public_key` must be valid for `public_key_len` bytes (must be 1952),
/// `message` for `message_len` bytes, and `signature` for `signature_len`
/// bytes (must be 3309); a pointer may be null only when its length is `0`.
...
/// verification error.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_verify(
    public_key: *const u8,
    ...
    ffi_guard(|| {
        ...
        match crypto_ml_dsa_65::verify_ml_dsa_65(public_key, message, signature) {
            Ok(()) => CRYPTO_OK,
...
/// Validates a 1952-byte ML-DSA-65 `public_key` and copies its canonical
/// encoding to `out`.
///
/// # Safety
///
/// `public_key` must be valid for `public_key_len` bytes (must be 1952; may be
/// null only when the length is `0`). `out` must be non-null and point to at
/// least `out_len` writable bytes (at least 1952). Returns [`CryptoStatus`]
/// via the return value: [`CRYPTO_OK`] on success, or [`CRYPTO_INVALID_KEY`]
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_encode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
...
/// Validates a 1952-byte encoded ML-DSA-65 `public_key` and copies the decoded
...
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_dsa_65_decode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
