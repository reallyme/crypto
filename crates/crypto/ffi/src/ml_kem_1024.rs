...
/// Length in bytes of an ML-KEM-1024 public key (1568).
pub const ML_KEM_1024_PUBLIC_KEY_LEN: usize = 1568;
/// Length in bytes of an ML-KEM-1024 secret key as handled here (64).
pub const ML_KEM_1024_SECRET_KEY_LEN: usize = 64;
/// Length in bytes of an ML-KEM-1024 ciphertext (1568).
pub const ML_KEM_1024_CIPHERTEXT_LEN: usize = 1568;
/// Length in bytes of an ML-KEM-1024 shared secret (32).
pub const ML_KEM_1024_SHARED_SECRET_LEN: usize = 32;
/// Length in bytes of ML-KEM deterministic encapsulation randomness.
pub const ML_KEM_1024_ENCAPS_RANDOMNESS_LEN: usize = 32;

/// Generates an ML-KEM-1024 keypair, writing the 1568-byte public key to
/// `public_out` and the 64-byte secret key to `secret_out`.
///
/// # Safety
///
/// `public_out` must point to at least `public_out_len` writable bytes (at
/// least 1568) and `secret_out` to at least `secret_out_len` writable bytes
...
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_generate_keypair(
    public_out: *mut u8,
    ...
    ffi_guard(|| {
        ...
        let (public, secret) =
            match crypto_ml_kem_1024::generate_ml_kem_1024_keypair().into_keypair_result() {
                Ok(value) => value,
...
/// Derives an ML-KEM-1024 keypair from a caller-supplied 64-byte FIPS seed.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_generate_keypair_from_seed(
    secret_key: *const u8,
    ...
    ffi_guard(|| {
        ...
        let secret_key = match <&[u8; ML_KEM_1024_SECRET_KEY_LEN]>::try_from(secret_key) {
            Ok(value) => value,
            Err(_) => return CRYPTO_INVALID_KEY,
        };
        let (public, secret) =
            match crypto_ml_kem_1024::generate_ml_kem_1024_keypair_from_seed(secret_key) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
        let status = unsafe { write_fixed(public_out, public_out_len, &public) };
...
/// Encapsulates to the ML-KEM-1024 `public_key`, writing the 1568-byte
...
/// `ciphertext_out_len` writable bytes (at least 1568) and `shared_secret_out`
...
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_encapsulate(
    public_key: *const u8,
    ...
    ffi_guard(|| {
        ...
        let (ciphertext, shared_secret) =
            match crypto_ml_kem_1024::ml_kem_1024_encapsulate(public_key) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
...
/// Deterministically encapsulates to an ML-KEM-1024 public key.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_encapsulate_derand(
    public_key: *const u8,
    public_key_len: usize,
    randomness: *const u8,
    ...
    ffi_guard(|| {
        ...
        if randomness.len() != ML_KEM_1024_ENCAPS_RANDOMNESS_LEN {
            return CRYPTO_INVALID_KEY;
        }
        let (ciphertext, shared_secret) =
            match crypto_ml_kem_1024::ml_kem_1024_encapsulate_derand(public_key, randomness) {
                Ok(value) => value,
                Err(_) => return CRYPTO_INVALID_KEY,
            };
        let status = unsafe { write_fixed(ciphertext_out, ciphertext_out_len, &ciphertext) };
...
/// Decapsulates an ML-KEM-1024 `ciphertext` with `secret_key`, writing the
/// recovered 32-byte shared secret to `shared_secret_out`.
///
/// # Safety
///
/// `ciphertext` must be valid for `ciphertext_len` bytes (must be 1568) and
...
/// zeroize it.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_decapsulate(
    ciphertext: *const u8,
    ...
    ffi_guard(|| {
        ...
        match crypto_ml_kem_1024::ml_kem_1024_decapsulate(ciphertext, secret_key) {
...
/// Validates a 1568-byte ML-KEM-1024 `public_key` and copies its canonical
/// encoding to `out`.
///
/// # Safety
///
/// `public_key` must be valid for `public_key_len` bytes (must be 1568; may be
/// null only when the length is `0`). `out` must be non-null and point to at
/// least `out_len` writable bytes (at least 1568). Returns [`CryptoStatus`]
/// via the return value: [`CRYPTO_OK`] on success, or [`CRYPTO_INVALID_KEY`]
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_encode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
...
/// Validates a 1568-byte encoded ML-KEM-1024 `public_key` and copies the
...
/// for a bad key length.
#[no_mangle]
pub unsafe extern "C" fn rm_crypto_ml_kem_1024_decode_public_key(
    public_key: *const u8,
    public_key_len: usize,
    out: *mut u8,
    out_len: usize,
