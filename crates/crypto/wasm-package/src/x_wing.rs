...
use crypto_x_wing::{
    ...
    generate_x_wing_768_keypair_derand, X_WING_1024_CIPHERTEXT_LEN, X_WING_1024_PUBLIC_KEY_LEN,
    X_WING_768_CIPHERTEXT_LEN, X_WING_768_PUBLIC_KEY_LEN, X_WING_ENCAPS_SEED_LEN,
    X_WING_SECRET_KEY_LEN,
};
...
#[wasm_bindgen(js_name = xWing1024GenerateKeypair)]
/// Generate a random X-Wing-1024 keypair for the TypeScript WASM provider.
pub fn x_wing_1024_generate_keypair() -> Result<JsValue, JsValue> {
    generate_keypair(generate_x_wing_1024_keypair)
}

#[wasm_bindgen(js_name = xWing1024DeriveKeypair)]
/// Derive an X-Wing-1024 public key from a 32-byte secret seed.
pub fn x_wing_1024_derive_keypair(secret_key: &Uint8Array) -> Result<JsValue, JsValue> {
    derive_keypair(secret_key, generate_x_wing_1024_keypair_derand)
}

#[wasm_bindgen(js_name = xWing1024Encapsulate)]
/// Randomly encapsulate to an X-Wing-1024 public key.
pub fn x_wing_1024_encapsulate(public_key: &Uint8Array) -> Result<JsValue, JsValue> {
    encapsulate(
        public_key,
        X_WING_1024_PUBLIC_KEY_LEN,
        crypto_x_wing::x_wing_1024_encapsulate,
    )
}

#[wasm_bindgen(js_name = xWing1024EncapsulateDerand)]
/// Deterministically encapsulate to an X-Wing-1024 public key for conformance tests.
pub fn x_wing_1024_encapsulate_derand(
    public_key: &Uint8Array,
    seed: &Uint8Array,
) -> Result<JsValue, JsValue> {
    encapsulate_derand(
        ...
        seed,
        X_WING_1024_PUBLIC_KEY_LEN,
        crypto_x_wing::x_wing_1024_encapsulate_derand,
    )
}

#[wasm_bindgen(js_name = xWing1024Decapsulate)]
/// Decapsulate an X-Wing-1024 ciphertext with a 32-byte secret seed.
pub fn x_wing_1024_decapsulate(
    ciphertext: &Uint8Array,
    secret_key: &Uint8Array,
) -> Result<Uint8Array, JsValue> {
    decapsulate(
        ciphertext,
        secret_key,
        X_WING_1024_CIPHERTEXT_LEN,
        crypto_x_wing::x_wing_1024_decapsulate,
    )
}
