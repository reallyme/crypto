...
/// Length in bytes of the serialized ML-KEM-768 seed-form secret key.
pub const ML_KEM_768_SECRET_KEY_LEN: usize = 64;

/// Validates an ML-KEM-768 public key and returns the original byte slice.
...
/// Encodes an ML-KEM-768 public key in raw FIPS 203 form.
pub fn encode_public_key(pk: &[u8]) -> Result<Vec<u8>, CryptoError> {
    Ok(assert_public_key(pk)?.to_vec())
}

/// Decodes an ML-KEM-768 public key from raw FIPS 203 form.
