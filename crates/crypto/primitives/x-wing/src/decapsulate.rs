...
use crate::suite::{
    XWingSuite, ML_KEM_SECRET_SEED_LEN, X25519_KEY_LEN, X_WING_1024_CIPHERTEXT_LEN,
...
fn split_ciphertext(
    ...
    let x25519_ciphertext = <&[u8; X25519_KEY_LEN]>::try_from(x25519_ciphertext)
...
fn ml_kem_768_decapsulate(
    ...
    let ciphertext =
        MlKem768Ciphertext::try_from(ciphertext).map_err(|_| CryptoError::InvalidKey)?;
...
fn ml_kem_1024_decapsulate(
    ...
    let ciphertext =
        MlKem1024Ciphertext::try_from(ciphertext).map_err(|_| CryptoError::InvalidKey)?;
    let mut shared_secret = decapsulation_key.decapsulate(&ciphertext);
    let shared_secret_bytes = Zeroizing::new(shared_secret.as_slice().to_vec());
    shared_secret.zeroize();
    Ok(shared_secret_bytes)
...
fn decapsulate(
    ...
    let mut x25519_shared_secret = x25519_shared.to_bytes();
...
/// Decapsulate an X-Wing-1024 ciphertext using the ReallyMe ML-KEM-1024 suite.
pub fn x_wing_1024_decapsulate(
    ...
    debug_assert_eq!(ciphertext.len(), X_WING_1024_CIPHERTEXT_LEN);
    Ok(result)
}
