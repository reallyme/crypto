...
use ml_kem::{
    ml_kem_1024::{EncapsulationKey as MlKem1024EncapsulationKey, MlKem1024},
...
use crate::suite::{
    XWingSuite, ML_KEM_SHARED_SECRET_LEN, X25519_KEY_LEN, X_WING_1024_CIPHERTEXT_LEN,
    X_WING_768_CIPHERTEXT_LEN, X_WING_ENCAPS_SEED_LEN, X_WING_SHARED_SECRET_LEN,
};
...
fn ml_kem_1024_encapsulate_derand(
    public_key: &[u8],
    randomness: &[u8; ML_KEM_SHARED_SECRET_LEN],
) -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), CryptoError> {
    let public_key = Key::<EncapsulationKey<MlKem1024>>::try_from(public_key)
        .map_err(|_| CryptoError::InvalidKey)?;
    let encapsulation_key =
        MlKem1024EncapsulationKey::new(&public_key).map_err(|_| CryptoError::InvalidKey)?;
    let randomness = B32::try_from(&randomness[..]).map_err(|_| CryptoError::InvalidKey)?;
    let (mut ciphertext, mut shared_secret) =
        encapsulation_key.encapsulate_deterministic(&randomness);
    let ciphertext_bytes = ciphertext.as_slice().to_vec();
...
fn encapsulate_derand(
    ...
    let mut ml_kem_randomness = [0u8; ML_KEM_SHARED_SECRET_LEN];
    ...
    let x25519_shared_secret = x25519_shared.to_bytes();
...
/// Encapsulate to an X-Wing-1024 public key using the ReallyMe ML-KEM-1024 suite.
pub fn x_wing_1024_encapsulate(
    public_key: &[u8],
) -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), CryptoError> {
    encapsulate(
        XWingSuite::MlKem1024,
        public_key,
        ml_kem_1024_encapsulate_derand,
    )
}

/// Deterministically encapsulate to an X-Wing-1024 public key using a 64-byte seed.
pub fn x_wing_1024_encapsulate_derand(
    ...
    debug_assert_eq!(result.0.len(), X_WING_1024_CIPHERTEXT_LEN);
    Ok(result)
}
