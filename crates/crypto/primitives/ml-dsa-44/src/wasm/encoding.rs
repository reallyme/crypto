...
/// Validates an ML-DSA-44 public key and returns the original byte slice.
...
/// Encodes an ML-DSA-44 public key in raw FIPS 204 form.
pub fn encode_public_key(pk: &[u8]) -> Result<Vec<u8>, CryptoError> {
    Ok(assert_public_key(pk)?.to_vec())
}

/// Decodes an ML-DSA-44 public key from raw FIPS 204 form.
