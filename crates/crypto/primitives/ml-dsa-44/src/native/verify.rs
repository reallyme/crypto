...
use ml_dsa::{EncodedVerifyingKey, MlDsa44, Signature, Verifier, VerifyingKey};

/// Verify an ML-DSA-44 detached signature.
///
/// - `pub_bytes` MUST be exactly 1312 bytes
/// - `sig_bytes` MUST be exactly 2420 bytes
/// - Raw message, no hashing, no context
pub fn verify_ml_dsa_44(pub_bytes: &[u8], msg: &[u8], sig_bytes: &[u8]) -> Result<(), CryptoError> {
    let public_key =
        EncodedVerifyingKey::<MlDsa44>::try_from(pub_bytes).map_err(|_| CryptoError::InvalidKey)?;
    let verifying_key = VerifyingKey::<MlDsa44>::decode(&public_key);
    let sig = Signature::<MlDsa44>::try_from(sig_bytes).map_err(|_| CryptoError::Signature {
