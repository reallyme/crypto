...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(js_name = generateMlDsa65Keypair)]
    fn js_generate_ml_dsa_65_keypair() -> JsValue;
}
...
///
/// Returns `(public_key[1952], secret_seed[32])`. Typed `CryptoError`
...
pub fn generate_ml_dsa_65_keypair() -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), CryptoError> {
    let v = js_generate_ml_dsa_65_keypair();
    ...
    if public.len() != 1952 || secret_seed.len() != 32 {
