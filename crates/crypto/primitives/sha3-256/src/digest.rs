...
/// SHA3-256 digest bytes.
#[derive(Clone, PartialEq, Eq, Zeroize, ZeroizeOnDrop)]
pub struct Sha3_256Digest {
    bytes: [u8; SHA3_256_DIGEST_LENGTH],
}

impl Sha3_256Digest {
    /// Borrow the fixed-size digest bytes.
    pub fn as_bytes(&self) -> &[u8; SHA3_256_DIGEST_LENGTH] {
        &self.bytes
    }

    /// Consume the digest and return its fixed-size byte array.
    pub fn into_bytes(self) -> [u8; SHA3_256_DIGEST_LENGTH] {
        self.bytes
    }
}
...
impl core::fmt::Debug for Sha3_256Digest {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "Sha3_256Digest(len={})", SHA3_256_DIGEST_LENGTH)
    }
}

/// Compute SHA3-256 digest bytes for `message`.
