...
impl SignatureAlgorithm for P256Algo {
    const ALG: Algorithm = Algorithm::P256;
    ...
    fn generate_keypair() -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), AlgorithmError> {
        #[cfg(any(feature = "native", all(feature = "wasm", target_arch = "wasm32")))]
        {
            return crate::algorithms::KeypairResultExt::into_algorithm_keypair(
                crypto_p256::generate_p256_keypair(),
                Self::ALG,
            );
        }
    ...
    fn derive_keypair(secret: &[u8]) -> Result<(Vec<u8>, Zeroizing<Vec<u8>>), AlgorithmError> {
        #[cfg(any(feature = "native", all(feature = "wasm", target_arch = "wasm32")))]
        {
            ...
            return crypto_p256::generate_p256_keypair_from_secret_key(secret_key)
    ...
    fn sign(secret: &[u8], msg: &[u8]) -> Result<Vec<u8>, AlgorithmError> {
        #[cfg(any(feature = "native", all(feature = "wasm", target_arch = "wasm32")))]
        {
            return crypto_p256::sign_p256_der_prehash(secret, msg).map_err(AlgorithmError::from);
        }
    ...
    fn verify(public: &[u8], msg: &[u8], sig: &[u8]) -> Result<(), AlgorithmError> {
        #[cfg(any(feature = "native", all(feature = "wasm", target_arch = "wasm32")))]
        {
            return crypto_p256::verify_p256_der_prehash(sig, msg, public)
...
impl P256Algo {
    /// Derive the P-256 ECDH shared secret; the returned value zeroizes on
    ...
    pub fn derive_shared_secret(
        ...
        #[cfg(any(feature = "native", all(feature = "wasm", target_arch = "wasm32")))]
        {
            return crypto_p256::derive_p256_shared_secret(secret_key, public_key)
                .map_err(AlgorithmError::from);
        }
