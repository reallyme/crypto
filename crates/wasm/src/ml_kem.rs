...
pub fn ml_kem_512_derive_keypair(secret_key: &Uint8Array) -> Result<JsValue, JsValue> {
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_KEM_SECRET_KEY_LEN)?);
...
pub fn ml_kem_512_encapsulate(public_key: &Uint8Array) -> Result<JsValue, JsValue> {
    let public_key = copy_exact(public_key, ML_KEM_512_PUBLIC_KEY_LEN)?;
...
pub fn ml_kem_512_encapsulate_derand(
    ...
    let public_key = copy_exact(public_key, ML_KEM_512_PUBLIC_KEY_LEN)?;
    let randomness = Zeroizing::new(copy_exact(randomness, ML_KEM_ENCAPS_RANDOMNESS_LEN)?);
    let (ciphertext, shared_secret) =
...
pub fn ml_kem_512_decapsulate(
    ...
    let ciphertext = copy_exact(ciphertext, ML_KEM_512_CIPHERTEXT_LEN)?;
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_KEM_SECRET_KEY_LEN)?);
...
pub fn ml_kem_768_encapsulate(public_key: &Uint8Array) -> Result<JsValue, JsValue> {
    let public_key = copy_exact(public_key, ML_KEM_768_PUBLIC_KEY_LEN)?;
    let (ciphertext, shared_secret) =
...
pub fn ml_kem_768_decapsulate(
    ...
    let ciphertext = copy_exact(ciphertext, ML_KEM_768_CIPHERTEXT_LEN)?;
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_KEM_SECRET_KEY_LEN)?);
...
pub fn ml_kem_1024_encapsulate(public_key: &Uint8Array) -> Result<JsValue, JsValue> {
    let public_key = copy_exact(public_key, ML_KEM_1024_PUBLIC_KEY_LEN)?;
    let (ciphertext, shared_secret) =
...
pub fn ml_kem_1024_decapsulate(
    ...
    let ciphertext = copy_exact(ciphertext, ML_KEM_1024_CIPHERTEXT_LEN)?;
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_KEM_SECRET_KEY_LEN)?);
