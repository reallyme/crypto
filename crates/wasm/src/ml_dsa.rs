...
pub fn ml_dsa_44_generate_keypair() -> Result<JsValue, JsValue> {
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_44::generate_ml_dsa_44_keypair().map_err(map_crypto_error)?;
...
pub fn ml_dsa_44_derive_keypair(secret_key: &Uint8Array) -> Result<JsValue, JsValue> {
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_DSA_SECRET_KEY_LEN)?);
    let secret_key = <&[u8; ML_DSA_SECRET_KEY_LEN]>::try_from(secret_key.as_slice())
        .map_err(|_| invalid_input())?;
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_44::generate_ml_dsa_44_keypair_from_seed(secret_key)
...
pub fn ml_dsa_44_sign(
    ...
    let signature = crypto_runtime::ml_dsa_44::sign_ml_dsa_44(&secret_key, &message)
        .map_err(map_crypto_error)?;
...
pub fn ml_dsa_44_verify(
    ...
    let public_key = copy_exact(public_key, ML_DSA_44_PUBLIC_KEY_LEN)?;
    let signature = copy_exact(signature, ML_DSA_44_SIGNATURE_LEN)?;
    let message = Zeroizing::new(copy_bounded(message, MAX_WASM_INPUT_LENGTH)?);
    crypto_runtime::ml_dsa_44::verify_ml_dsa_44(&public_key, &message, &signature)
        .map_err(map_crypto_error)
}
...
pub fn ml_dsa_65_generate_keypair() -> Result<JsValue, JsValue> {
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_65::generate_ml_dsa_65_keypair().map_err(map_crypto_error)?;
...
pub fn ml_dsa_65_derive_keypair(secret_key: &Uint8Array) -> Result<JsValue, JsValue> {
    ...
    let secret_key = <&[u8; ML_DSA_SECRET_KEY_LEN]>::try_from(secret_key.as_slice())
        .map_err(|_| invalid_input())?;
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_65::generate_ml_dsa_65_keypair_from_seed(secret_key)
            .map_err(map_crypto_error)?;
    keypair_to_js(public_key, secret_key, ML_DSA_65_PUBLIC_KEY_LEN)
}
...
pub fn ml_dsa_65_sign(
    ...
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_DSA_SECRET_KEY_LEN)?);
    let message = Zeroizing::new(copy_bounded(message, MAX_WASM_INPUT_LENGTH)?);
    let signature = crypto_runtime::ml_dsa_65::sign_ml_dsa_65(&secret_key, &message)
        .map_err(map_crypto_error)?;
...
pub fn ml_dsa_65_verify(
    ...
    let public_key = copy_exact(public_key, ML_DSA_65_PUBLIC_KEY_LEN)?;
    let signature = copy_exact(signature, ML_DSA_65_SIGNATURE_LEN)?;
    let message = Zeroizing::new(copy_bounded(message, MAX_WASM_INPUT_LENGTH)?);
    crypto_runtime::ml_dsa_65::verify_ml_dsa_65(&public_key, &message, &signature)
        .map_err(map_crypto_error)
}
...
pub fn ml_dsa_87_generate_keypair() -> Result<JsValue, JsValue> {
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_87::generate_ml_dsa_87_keypair().map_err(map_crypto_error)?;
...
pub fn ml_dsa_87_derive_keypair(secret_key: &Uint8Array) -> Result<JsValue, JsValue> {
    ...
    let secret_key = <&[u8; ML_DSA_SECRET_KEY_LEN]>::try_from(secret_key.as_slice())
        .map_err(|_| invalid_input())?;
    let (public_key, secret_key) =
        crypto_runtime::ml_dsa_87::generate_ml_dsa_87_keypair_from_seed(secret_key)
            .map_err(map_crypto_error)?;
    keypair_to_js(public_key, secret_key, ML_DSA_87_PUBLIC_KEY_LEN)
}
...
pub fn ml_dsa_87_sign(
    ...
    let secret_key = Zeroizing::new(copy_exact(secret_key, ML_DSA_SECRET_KEY_LEN)?);
    let message = Zeroizing::new(copy_bounded(message, MAX_WASM_INPUT_LENGTH)?);
    let signature = crypto_runtime::ml_dsa_87::sign_ml_dsa_87(&secret_key, &message)
        .map_err(map_crypto_error)?;
...
pub fn ml_dsa_87_verify(
    ...
    let public_key = copy_exact(public_key, ML_DSA_87_PUBLIC_KEY_LEN)?;
    let signature = copy_exact(signature, ML_DSA_87_SIGNATURE_LEN)?;
    let message = Zeroizing::new(copy_bounded(message, MAX_WASM_INPUT_LENGTH)?);
    crypto_runtime::ml_dsa_87::verify_ml_dsa_87(&public_key, &message, &signature)
        .map_err(map_crypto_error)
}
